...
/**
 * Main component to render the sidebar for chapters and sections of a course.
 * Includes dynamic loading, progress tracking, and section toggling.
 */
const ChaptersSidebar = () => {
  const router = useRouter();
  const { setOpen } = useSidebar(); // Custom hook to control sidebar state
  const [expandedSections, setExpandedSections] = useState<string[]>([]); // Tracks expanded sections

  // Fetch course progress data using a custom hook
  const {
    user,
    course,
    userProgress,
  ...
  const sidebarRef = useRef<HTMLDivElement>(null); // Reference for the sidebar container

  useEffect(() => {
    setOpen(false); // Close sidebar on initial load
  ...
  // Navigates to a specific chapter within the course.
...
/**
 * Renders a single section with its chapters and progress.
 */
const Section = ({
  section,
  index,
...
/**
 * Visual representation of progress within a section.
 */
...
/**
 * Renders the list of chapters within a section.
 */
const ChaptersList = ({
  section,
  sectionProgress,
...
/**
 * Renders an individual chapter with its title and progress status.
 */
const Chapter = ({
  chapter,
  index,
  ...
  return (
    ...
    >
      {isCompleted ? (
        ...
        <div className="chapters-sidebar__chapter-number">{index + 1}</div>
      )}
      <span className="chapters-sidebar__chapter-title">{chapter.title}</span>
